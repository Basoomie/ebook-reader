<script lang="ts">
  ...
  const isVertical =
    (window.localStorage.getItem('writingMode') || 'vertical-rl') === 'vertical-rl';
  ...
  $: $isRecording$ =
    $exportAudioProcessor$ === AudioProcessor.RECORDER && !!$exportCancelController$;

  $: enabledFooterActions = $actionListOfFooter$.filter((action) => action.enabled);

  $: showCancelFooterAction =
    !!$exportCancelController$ &&
    !!enabledFooterActions.find(
      ({ action }) => action === Action.EXPORT_NEW || action === Action.EXPORT_UPDATE
    );
  ...
  $: if (
    readerMenuRangeElement &&
    $readerMenuPauseMode$ === ReaderMenuPauseMode.PAUSE &&
    !$isRecording$
  ) {
    $paused$ = true;
  }
  ...
  async function initializeComponent() {
    ...
    try {
      ...
      $extensionData$ = {
        title: book.title,
        lastAudio:
          $filesystemApiEnabled$ && audioHandle?.handle instanceof FileSystemFileHandle
            ? audioHandle.handle
            : undefined,
        lastSubtitle:
          $filesystemApiEnabled$ && subtitleHandle?.handle instanceof FileSystemFileHandle
  ...
  async function initializeFiles(retry = true) {
    const lastSubtitle =
      !$currentSubtitleFile$ && $extensionData$.lastSubtitle
        ? $extensionData$.lastSubtitle
        : undefined;
    let lastAudio =
      !$currentAudioSourceUrl$ && $extensionData$.lastAudio ? $extensionData$.lastAudio : undefined;
  ...
  async function loadPersistedSubtitles(resetIsLoading = false) {
    ...
    try {
      ...
      for (
        let index = 0, { length } = $extensionData$.subtitleData.subtitles;
        index < length;
        index += 1
      ) {
        ...
        const startSeconds = Math.max(
          ...
          subtitle.adjustedStartSeconds ??
            subtitle.originalStartSeconds + subtitlesGlobalStartPadding
        );
  ...
  function resetReaderMenu(resetAll = true) {
    const lastActiveElements = bookContentElement.querySelectorAll(
      `${getLineCSSSelector()}.menu-open`
    );
  ...
  function handleReaderInteraction() {
    ...
    if (
      $readerMenuOpenMode$ === ReaderMenuOpenMode.DISABLED &&
      $readerClickAction$ !== Action.NONE
    ) {
    ...
    const subtitleElements = [
      ...bookContentElement.querySelectorAll(getLineCSSSelectorForId(subtitleId!))
    ];
      ...
      <div title="Getting started" class:m-l-b={isLeftMenu} class:m-r-b={!isLeftMenu}>
        <Popover>
          ...
          <ol class="getting-started">
            ...
            <li>
              Switch to "Match" Tab, match the subtitle against your book and click on "Save &
              reload page" to enable full functionality like reader highlighting, reader menu,
              autoscroll etc. after the page was reloaded
            </li>
            <li>Configure your experience to your preferences by going to the settings tab</li>
            <li>
              Read <a href="https://github.com/Renji-XD/ttu-whispersync" target="_blank"
                >FAQ & more</a
