<script lang="ts">
  ...
  async function onMatchSubtitles() {
    ...
    try {
      ...
      let { currentSubtitle, currentSubtitleLength } = getSubtitleData(
        subtitles,
      ...
      const bookTextWalker = document.createTreeWalker(bookHTML, NodeFilter.SHOW_TEXT, {
        acceptNode(node) {
          if (hasHint && !passedStartNode) {
            passedStartNode =
              startHintNodeContent === node.textContent &&
              getTTUParent(node)?.id === startHintParentId;
      ...
      while (currentTextNodeIndex < textNodeCount && currentSubtitleIndex < subtitles.length) {
        ...
        if (currentNormalizedTextLength >= currentSubtitleLength) {
          ...
          const isThresholdMet =
            similiarityResult.bestLineSimiliarityValue >= $matchLineSimilarityThreshold$;
      ...
      for (let index = 0, { length } = bookTextEntries; index < length; index += 1) {
        ...
        if (originalElement !== matchedElement) {
          throwMatchError(
            `element mismatch on index ${index}`,
            originalElement,
            matchedElement,
            ttuParent
          );
        ...
        for (
          let index2 = 0, { length: length2 } = bookTextCharacters;
          index2 < length2;
          index2 += 1
        ) {
  ...
  function onDownloadSubtitles() {
    ...
    downloadFile(
      document,
      new Blob([subtitleContent], {
        type: `${isIOS ? 'text/plain' : 'application/x-subrip'};charset=utf-8`
      }),
  ...
  async function onSaveMatch() {
    ...
    try {
      // Start from the row as it currently is so any field this component does
      // not know about is carried over untouched, then overlay the authoritative
      // book data. This row gets replicated back onto the external archive, so
      // dropping a field here deletes it there as well.
      const currentData = await $booksDB$.get('data', $bookData$.id);
      const newData = {
        ...currentData,
        ...$bookData$,
  ...
  function findBestSimilarity(
    ...
    while (
      currentNormalizedTextLength <= currentSubtitleLength &&
      newTextNodeIndex < textNodes.length
    ) {
...
<div class="flex-1">
  <div class="settings-grid">
    ...
    <Popover>
      ...
      <div>
        Indicates # of lines which passed the similiarity check but are not equal to the book text
      </div>
    </Popover>
...
<div class="flex justify-end match-btns">
  {#if maxProgress}
    <button class="btn m-r-s" disabled={cancelSignal.aborted} on:click={onCancelMatch}
      >Cancel</button
    >
  {/if}
  ...
  <Popover placement="top">
    ...
    <div>
      By matching the subtitles to the book you can enable extra functionality like the reader menu,
      auto scroll etc.
    </div>
  </Popover>
</div>
