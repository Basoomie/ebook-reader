<script lang="ts">
  ...
  import type { FileInstance, EventWithElement, FileResult, FileExtension } from '../lib/dropzone';
  ...
  function onTriggerFileSelection(): void {
    if (disabled) {
      return;
    }
  ...
  function onDrop({ dataTransfer }: DragEvent) {
    ...
    for (let index = 0, { length } = items; index < length; index += 1) {
      ...
      if (item.kind === 'file') {
        ...
        const entry = useNativeFileSystem
  ...
  async function getFilesFromFileSystem() {
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      collectFiles(files.map((item: any) => Promise.resolve(item)));
    } catch (error: any) {
  ...
  async function processEntry(entry: FileSystemHandle | FileSystemEntry, currentPath = '/') {
    ...
    if (useNativeFileSystem && entry instanceof FileSystemFileHandle) {
      ...
      return result;
    } else if (useNativeFileSystem && entry instanceof FileSystemDirectoryHandle) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const iterator = (entry as any).entries();

      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      let entries = await iterator.next().catch(({ message }: any) => {
      ...
      while (!entries.done) {
        ...
        entries =
          !result.files.length || multiple
            ? await iterator.next().catch(({ message }: any) => {
                result.errors.push(`Error reading directory ${entry.name} - ${message}`);
  ...
  function isFileSystemDirectoryEntry(
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return (entry as any).isDirectory;
  }
