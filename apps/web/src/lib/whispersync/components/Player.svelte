<script lang="ts">
  ...
  import { releaseRecorderGraph, startRecording, stopRecording } from '../lib/recorder';
  ...
  export async function onScrollToSubtitle() {
    ...
    let targetSubtitle = $currentSubtitles$.get(
      $activeSubtitle$.current || $activeSubtitle$.previous
    );
  ...
  $: altRewindTitle = $isRecording$
    ? 'Recording in progress'
    : `Rewind ${$playerAltRewindTime$} seconds`;

  $: fastForwardTitle = $isRecording$
    ? 'Recording in progress'
    : `Fast-Forward ${$playerFastForwardTime$} seconds`;
    ...
    if (event.altKey) {
      ...
      switch (actionKey) {
        ...
        case 'arrowdown':
          ...
          break;
        case 'arrowright':
          targetSubtitle = getDummySubtitle(
            Math.min($duration$, $currentTime$ + $playerFastForwardTime$)
          );
          action = Action.RESTART_PLAYBACK;
          keepPauseState = true;
        ...
        case 'arrowup':
          targetSubtitle = getDummySubtitle(
            Math.min($duration$, $currentTime$ + $playerAltFastForwardTime$)
          );
          action = Action.RESTART_PLAYBACK;
          keepPauseState = true;
  ...
  async function onCurrentTimeChange() {
    ...
    try {
      ...
      document.dispatchEvent(
        new CustomEvent('ttu-action', { detail: { type: 'sync', syncType: 'audioBook' } })
      );
    } catch ({ message }: any) {
  ...
  function updateTextTrack(subtitleChange: SubtitleChange): void {
  ...
  async function updateCSSClasses(lastActiveId?: string | Event) {
    ...
    if (activeTrack && !activeCues.length && typeof lastActiveId === 'string' && isIOS) {
      const fallbackId =
        [...$currentSubtitles$.values()].findLast(
          (subtitle) => $currentTime$ >= subtitle.startSeconds
        )?.id || '';
    ...
    if ($paused$ && activeCues.length > 1) {
      ...
      if (
        (subtitle1.startSeconds >= subtitle2.startSeconds &&
          subtitle1.startSeconds <= subtitle2.endSeconds) ||
        (subtitle2.startSeconds >= subtitle1.startSeconds &&
          subtitle2.startSeconds <= subtitle1.endSeconds)
      ) {
  ...
  function adjustPlaybackRateBy(value: number) {
    $playbackRate$ = between(
      0.1,
      2,
      Math.round(($playbackRate$ + value) * 100 + Number.EPSILON) / 100
    );
...
<div class="flex items-center w-full m-t-b" class:invisible={!$currentAudioLoaded$}>
  ...
  <Popover
    ...
    bind:this={playbackRatesPopover}
  >
