...
interface BooksDbData {
  ...
  htmlBackup?: string;
  // Declared so they survive the object spreads in AudioBookMenu/Match/Audiobook.
  // The local row of a storage-source book is a placeholder that has these
  // empty - writing them back would wipe them from the external archive.
  styleSheet?: string;
  sections?: unknown[];
  characters?: number;
