...
async function verifyAnkiSettings(
  ...
  try {
    ...
    lastError$.set(
      `Failed to verify Anki settings for card ${isUpdate ? 'update' : 'creation'}: ${message}`
    );
...
export async function exportToAnki(
  ...
  const useUpdateStores =
    isUpdate &&
    (!!get(settings$.ankiUpdateSentenceField$) || !!get(settings$.ankiUpdateSoundField$));
  ...
  const sentenceFieldStore = useUpdateStores
    ? settings$.ankiUpdateSentenceField$
    : settings$.ankiSentenceField$;
  const soundFieldStore = useUpdateStores
    ? settings$.ankiUpdateSoundField$
    : settings$.ankiSoundField$;
  const coverFieldStore = useUpdateStores
    ? settings$.ankiUpdateCoverField$
    : settings$.ankiCoverField$;
    ...
    try {
      ...
      if (ankiSoundField && canCaptureAudio) {
        if (isFFMPEG) {
          ...
          audioBuffer = await new Promise<ArrayBuffer | undefined>(
            (recorderSuccess, recorderFailure) =>
              playLine$.set({
                subtitles,
      ...
      for (let index2 = 0, { length: length2 } = ankiFields; index2 < length2; index2 += 1) {
        ...
        if (processAsSentenceField || processAsAudioField || processAsCoverField) {
          ...
          cardData.note.fields[field] = getNewFieldValue(
            exportFieldMode,
            existingFieldValue,
            newFieldValue
          );
